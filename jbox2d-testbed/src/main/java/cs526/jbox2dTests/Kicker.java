...
public class Kicker extends AutoLoadedTest {
	...
	public void initTest(boolean argDeserialized) {
		...
		bd.position = new Vec2(.2f, radius);
		...
		virtualControls.put("l_up_leg",
				new VirtualPdController(model.getLinkByName("l_up_leg"),
						0.125f, 0.025f));
		virtualControls.put("l_bottom_leg",
				new VirtualPdController(model.getLinkByName("l_bottom_leg"),
						0.125f, 0.025f));
		virtualControls.put("r_up_leg",
				new VirtualPdController(model.getLinkByName("r_up_leg"),
						0.125f, 0.025f));
		
		virtualControls.put("l_foot",
				new VirtualPdController(model.getLinkByName("l_foot"),
						0.125f, 0.025f));
	...
	public synchronized void step(TestbedSettings settings) {
		...
		float m_angle = (float) Math.toRadians(-60);
		...
		float compensateAngle = compensateAngle(2 * com - stanceAnkleX
				- swingAnkleX, velocity);
		...
		if (getStepCount() <= 600 && getStepCount() % 60 == 0)
			...
			writer.printf("%2.3f %2.3f %2.3f %2.3f %2.3f %2.3f %2.3f\n",
					stanceAnkleX, swingAnkleX, com, com - stanceAnkleX, com
							- swingAnkleX, velocity, compensateAngle);
		...
		case 0:
			...
			virtualTorque += virtualControls.get("l_bottom_leg").moveTo(zero);

//			lat = model.getControllerByName("l_ankle").torque;

			
			virtualTorque += virtualControls.get("r_up_leg").moveTo(zero);
			virtualTorque += virtualControls.get("torso").moveTo(zero);
//			rnt = model.getControllerByName("r_knee").torque;
			
			
			
//			float vt2 = rut + rnt;
			
//			getTorso().applyTorque(-vt1 - vt2);
...
//			lat = model.getControllerByName("l_ankle").torque;
			...
			virtualControls.get("torso").moveTo(m_angle/2);
//			getTorso().applyTorque(-virtualTorque - lat - rnt);
		...
		case 2:
			...
			virtualTorque += virtualControls.get("l_bottom_leg").moveTo(zero);
			virtualTorque += virtualControls.get("r_up_leg").moveTo(-m_angle);
//			virtualTorque += virtualControls.get("l_foot").moveTo(-m_angle);

//			lat = model.getControllerByName("l_ankle").torque;
			rnt = model.getControllerByName("r_knee").torque;

			virtualTorque += virtualControls.get("torso").moveTo(0);
